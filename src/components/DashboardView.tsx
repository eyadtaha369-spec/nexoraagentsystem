...
export function DashboardView({ mode }: { mode: "admin" | "agent" }) {
  ...
  const { data: allLeads = [] } = useQuery({ queryKey: ["leads"], queryFn: () => listFn() });

  const isAdmin = mode === "admin";
  // Defense-in-depth: in agent mode, scope client-side to leads assigned to this user's tab,
  // in addition to RLS filtering on the server.
  const myTab = me?.profile?.sheet_tab_name;
  const leads = isAdmin
    ? allLeads
    : (allLeads as any[]).filter((l) => myTab && l.assigned_agent === myTab);
  ...
  const byAgent: Record<string, { total: number; won: number }> = {};
  leads.forEach((l: any) => {
    const a = l.assigned_agent || "Unassigned";
    if (!byAgent[a]) byAgent[a] = { total: 0, won: 0 };
    byAgent[a].total++;
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const count = (leads as any[]).filter((l) => (l.date_added || "").slice(0,10) === key).length;
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm text-muted-foreground uppercase tracking-[0.2em]">
            {isAdmin ? "Admin overview — all agents" : "My performance"}
          </p>
